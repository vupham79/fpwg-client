...
import { Grid, Divider, IconButton, InputBase, Paper, Button, Dialog, TextField } from "@material-ui/core";
...
class TableTheme extends Component {
  state = {
    ...
    showDialog: false,
    selectedItem: { id: '0', mainColor: '#212121', name: '', fontBody: '', fontTitle: '' },
    updateData: { id: '0', mainColor: '#212121', name: '', fontBody: '', fontTitle: '' }
  };
  ...
  setOpenDialogue = (item) => {
    ...
    this.state.updateData.mainColor = item.mainColor;
    this.state.updateData.fontBody = item.fontBody;
    this.state.updateData.fontTitle = item.fontTitle;
    this.state.updateData.name = item.name;
  };
  ...
  handleChangeName = text => {
    this.state.updateData.name = text.target.value;
    this.setState({
  ...
  handleChangeColor = color => {
    this.state.updateData.mainColor = color.hex;
    this.setState({
      updateData: this.state.updateData,
    }); //update render
  };
  handleChangeFontTitle = font => {
    this.state.updateData.fontTitle = font.family;
  };
  handleChangeFontBody = font => {
    this.state.updateData.fontBody = font.family;
  };

  handleUpdate = body => {
    this.props.updateTheme(this.state.updateData.id, this.state.updateData.name, this.state.updateData.fontBody, this.state.updateData.fontTitle, this.state.updateData.mainColor)
  };
  ...
  render() {
    ...
    return (
      <React.Fragment>
              ...
              <div key={row.id}>
                <Grid container direction="row">
                  ...
                  <Grid container item xs={2}>
                    <Grid item xs={1}>
                      <div style={{ backgroundColor: row.mainColor, height: 18 }} />
                    </Grid>
                    <Grid item xs={1}>
                    </Grid>
                    <Grid item xs={3}>
                  ...
                  <Grid item xs={1}>
                    <Button color="primary" onClick={() => this.setOpenDialogue(row)}>Edit</Button>
                  </Grid>
                </Grid>
                <Divider />
              </div>
        ...
        <Dialog open={this.state.showDialog} onClose={() => this.setCloseDialogue()} maxWidth="sm" fullWidth>

          <Grid container xs={12} justify="center" alignContent="center" style={{ padding: 10, overflowY: 'scroll' }}>
            ...
            <Grid item xs={12}>
              <p style={{ textAlign: "left", fontWeight: "bold" }}>Font Title</p>
            </Grid>
            ...
            <Grid item xs={12}>
              <p style={{ textAlign: "left", fontWeight: "bold" }}>Main color</p>
            </Grid>
          ...
          <Button fullWidth color="primary" variant="contained" style={{ postion: 'fixed' }} onClick={() => this.handleUpdate(this.state.updateData)}>Update</Button>
        </Dialog>
      </React.Fragment>
    );
  }
...
const mapDispatchToProps = dispatch => ({
  ...
  updateTheme: (id, name, fontBody, fontTitle, mainColor) => dispatch(updateTheme(id, name, fontBody, fontTitle, mainColor))
});
